...
template <typename T>
inline void EigenQuaternionToAngleAxis(const T* eigen_quaternion, T* angle_axis) {
  const T quaternion[4] = {eigen_quaternion[3], eigen_quaternion[0], eigen_quaternion[1],
                           eigen_quaternion[2]};
...
struct PoseGraphRelativeCost {
 public:
  explicit PoseGraphRelativeCost(const Rigid3d& j_from_i, const Matrix6d covariance)
      : j_from_i_(j_from_i), sqrt_information_(covariance.inverse().llt().matrixL()) {}

  static ceres::CostFunction* Create(const Rigid3d& j_from_i, const Matrix6d covariance) {
    return (new ceres::AutoDiffCostFunction<PoseGraphRelativeCost, 6, 4, 3, 4, 3>(
        new PoseGraphRelativeCost(j_from_i, covariance)));
  }

  template <typename T>
  bool operator()(const T* const i_from_world_q, const T* const i_from_world_t,
                  const T* const j_from_world_q, const T* const j_from_world_t,
                  T* residuals_ptr) const {
    const Eigen::Quaternion<T> i_from_j_q =
        EigenQuaternionMap<T>(i_from_world_q) *
        EigenQuaternionMap<T>(j_from_world_q).inverse();
    Eigen::Matrix<T, 3, 1> i_from_j_t = EigenVector3Map<T>(i_from_world_t) -
                                        i_from_j_q * EigenVector3Map<T>(j_from_world_t);

    const Eigen::Quaternion<T> mes_from_est_q = j_from_i_.rotation.cast<T>() * i_from_j_q;
    ...
    mes_from_est_t =
        j_from_i_.translation.cast<T>() + j_from_i_.rotation.cast<T>() * i_from_j_t;
...
struct PoseGraphAbsoluteCost {
 public:
  ...
  template <typename T>
  bool operator()(const T* const cam_from_world_q, const T* const cam_from_world_t,
                  T* residuals_ptr) const {
